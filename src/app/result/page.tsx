...
import { Check, ChevronDown, Share2, Link2, X, RefreshCw, Crown, Calendar, Gift, Printer } from "lucide-react";
  ...
  if (notFound) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center px-6 text-center">
        ...
        <h1 className="text-2xl font-bold text-zinc-100">{t("result.not_found")}</h1>
        <p className="mt-2 text-zinc-400">{t("result.not_found_desc")}</p>
        <Link href="/diagnose" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-violet-500 to-blue-500 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-violet-500/25">{t("result.new_start")}</Link>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-3xl px-4 pt-24 sm:px-6">
        ...
        <motion.section {...fadeUp} transition={{ duration: 0.5 }}>
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-zinc-100 sm:text-3xl">{t("result.title")}</h1>
            <button
              onClick={() => window.print()}
              className="no-print flex items-center gap-2 rounded-lg border border-zinc-600 px-3 py-2 text-xs text-zinc-300 transition-colors hover:bg-zinc-800/60"
            >
              <Printer size={14} />
              {locale === "en" ? "PDF" : "PDF 저장"}
            </button>
          </div>
          <p className="mt-3 text-base text-zinc-400 leading-relaxed">{result.summary}</p>
          <div className="mt-4 flex flex-wrap gap-2">
            {country && <span className="inline-flex items-center gap-1 rounded-full bg-zinc-800/60 border border-zinc-700/50 px-3 py-1 text-xs text-zinc-200">{country.flag} {locale === "en" ? country.name_en : country.name_ko}</span>}
            {industry && <span className="inline-flex items-center gap-1 rounded-full bg-zinc-800/60 border border-zinc-700/50 px-3 py-1 text-xs text-zinc-200">{industry.icon} {locale === "en" ? industry.label_en : industry.label_ko}</span>}
            {Object.entries(result.input.answers).map(([k, v]) => (<span key={k} className="rounded-full bg-zinc-800/60 border border-zinc-700/50 px-3 py-1 text-xs text-zinc-400">{v}</span>))}
          </div>
        </motion.section>

        {/* Cost */}
        <motion.section {...fadeUp} transition={{ duration: 0.5, delay: 0.1 }} className="mt-8">
          <div className="rounded-2xl border border-violet-500/30 bg-gradient-to-r from-violet-500/10 to-blue-500/10 p-6 md:p-8">
            <p className="text-sm text-zinc-400">{t("result.cost_label")}</p>
            <p className="mt-1 text-3xl font-bold text-accent">{result.totalEstimatedCost}</p>
            <p className="mt-2 text-xs text-zinc-500">{t("result.cost_note")}</p>
          </div>
        </motion.section>
        ...
        <motion.section {...fadeUp} transition={{ duration: 0.5, delay: 0.2 }} className="mt-10">
          ...
          <div className="space-y-4">
            {result.phases.map((phase, phaseIdx) => {
              ...
              return (
                <motion.div key={phaseIdx} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 * phaseIdx }} className="overflow-hidden rounded-2xl border border-zinc-700/50 bg-zinc-800/40" style={{ borderLeft: `4px solid ${phase.color}` }}>
                  <button onClick={() => togglePhase(phaseIdx)} className="flex w-full items-center gap-3 px-5 py-4 text-left transition-colors duration-200 hover:bg-zinc-800/30">
                    <span className="text-xl">{phase.icon}</span>
                    <div className="flex-1 min-w-0">
                      <span className="text-sm font-semibold">{phase.phase}</span>
                      <div className="mt-1 flex items-center gap-2">
                        <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-zinc-800/60"><div className="h-full rounded-full bg-accent transition-all duration-300" style={{ width: total > 0 ? `${(done / total) * 100}%` : "0%" }} /></div>
                        <span className="text-xs text-zinc-500 shrink-0">{done}/{total}</span>
                      </div>
                    </div>
                    <motion.div animate={{ rotate: isOpen ? 180 : 0 }} transition={{ duration: 0.2 }}><ChevronDown size={18} className="text-zinc-500" /></motion.div>
                  </button>
                  <AnimatePresence initial={false}>
                    {isOpen && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }} className="overflow-hidden">
                        <div className="border-t border-zinc-700/30 px-5 py-3 space-y-1">
        ...
        {result.taxCalendar?.length > 0 && (
          <motion.section {...fadeUp} transition={{ duration: 0.5, delay: 0.3 }} className="mt-10">
            ...
            <div className="rounded-2xl border border-zinc-700/50 bg-zinc-800/40 divide-y divide-white/5">
              {result.taxCalendar.map((tax, i) => (
                <div key={i} className="flex items-center gap-4 px-5 py-3.5">
                  <span className="shrink-0 w-10 text-sm font-semibold text-zinc-300">{tax.month}</span>
                  <span className="flex-1 text-sm text-zinc-400">{tax.event}</span>
          ...
          <motion.section {...fadeUp} transition={{ duration: 0.5, delay: 0.4 }} className="mt-10">
            ...
            <div className="space-y-3">
              {result.governmentSupport.map((prog, i) => (
                <div key={i} className="rounded-2xl border border-zinc-700/50 bg-zinc-800/40 p-5 md:p-6">
                  ...
                  <p className="mt-2 text-sm text-zinc-400 leading-relaxed">{prog.description}</p>
                </div>
              ))}
            </div>
          </motion.section>
        ...
        <motion.section {...fadeUp} transition={{ duration: 0.5, delay: 0.5 }} className="mt-10 text-center">
          <button onClick={() => setShowShareModal(true)} className="inline-flex items-center gap-2 rounded-xl border border-zinc-700/50 px-6 py-2.5 text-sm text-zinc-300 transition-all duration-200 hover:bg-zinc-800/60 hover:text-white">
        ...
        <motion.section {...fadeUp} transition={{ duration: 0.5, delay: 0.6 }} className="mt-12">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <Link href={`/register-guide${result.input.country ? `?country=${result.input.country}` : ""}`} className="group rounded-2xl border border-zinc-700/50 bg-zinc-800/40 p-6 transition-all duration-300 hover:border-violet-500/30 hover:-translate-y-1">
            ...
            <Link href="/location" className="group rounded-2xl border border-zinc-700/50 bg-zinc-800/40 p-6 transition-all duration-300 hover:border-violet-500/30 hover:-translate-y-1">
            ...
            <Link href="/diagnose" className="group rounded-2xl border border-zinc-700/50 bg-zinc-800/40 p-6 transition-all duration-300 hover:border-violet-500/30 hover:-translate-y-1">
              <span className="text-2xl">🔄</span>
      ...
      <AnimatePresence>
        {showShareModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-6" onClick={() => setShowShareModal(false)}>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} onClick={(e) => e.stopPropagation()} className="w-full max-w-sm rounded-2xl border border-zinc-700/50 bg-[#111118] p-6">
              <div className="flex items-center justify-between mb-5"><h3 className="text-base font-semibold">{t("result.share")}</h3><button onClick={() => setShowShareModal(false)} className="text-zinc-400 hover:text-white"><X size={18} /></button></div>
              <div className="space-y-3">
                <button onClick={handleCopyLink} className="flex w-full items-center gap-3 rounded-xl border border-zinc-700/50 bg-zinc-800/40 px-4 py-3 text-sm transition-colors duration-200 hover:bg-zinc-800/60">
                ...
                <button onClick={handleShareTwitter} className="flex w-full items-center gap-3 rounded-xl border border-zinc-700/50 bg-zinc-800/40 px-4 py-3 text-sm transition-colors duration-200 hover:bg-zinc-800/60">
...
function ChecklistRow({ item, itemKey, isChecked, isDescOpen, onToggleCheck, onToggleDesc, requiredLabel }: { item: ChecklistItem; itemKey: string; isChecked: boolean; isDescOpen: boolean; onToggleCheck: () => void; onToggleDesc: () => void; requiredLabel: string }) {
  return (
    <div className="rounded-xl transition-colors duration-200 hover:bg-zinc-800/30">
      <div className="flex items-start gap-3 px-3 py-2.5">
        <button onClick={onToggleCheck} className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded border transition-all duration-200 min-h-[44px] min-w-[44px] -m-2.5 p-2.5 ${isChecked ? "border-accent bg-accent/20" : "border-zinc-600 hover:border-white/40"}`}>
          {isChecked && <Check size={12} className="text-accent" />}
        </button>
        <button onClick={onToggleDesc} className="flex-1 min-w-0 text-left">
          <span className={`text-sm transition-all duration-200 ${isChecked ? "text-accent/60 line-through" : "text-zinc-200"}`}>{item.title}</span>
          <div className="mt-1 flex flex-wrap items-center gap-2">
            {item.tag && <span className="rounded bg-zinc-800/60 px-1.5 py-0.5 text-[10px] text-zinc-500">{item.tag}</span>}
        ...
        <motion.div animate={{ rotate: isDescOpen ? 180 : 0 }} transition={{ duration: 0.2 }} className="mt-1 shrink-0">
          <ChevronDown size={14} className="text-zinc-500 cursor-pointer" onClick={onToggleDesc} />
        </motion.div>
      </div>
      <AnimatePresence initial={false}>
        {isDescOpen && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
            <p className="px-11 pb-3 text-xs leading-relaxed text-zinc-500">{item.description}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
